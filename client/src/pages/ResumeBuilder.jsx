...
const ResumeBuilder = () => {
  ...
  const loadExistingResume = async() => {
    ...
    // console.log('dummyResumeData:', dummyResumeData)
  ...
  const saveResume=async()=>{
    try {
      ...
      //   imageToUpload = resumeData.personal_info.image
      // } else if(typeof resumeData.personal_info.image==='string' && removeBackground){
      //   // Existing image URL - fetch and convert to File for background removal
      //   try {
      //     const response = await fetch(resumeData.personal_info.image)
      //     const blob = await response.blob()
      //     imageToUpload = new File([blob], 'resume.png', { type: 'image/png' })
      //   } catch (err) {
      ...
      //   removeBackground && formData.append("removeBackground","yes")
      //   console.log('Uploading image with removeBackground:', removeBackground)
      // } else {
          ...
          <div className='grid lg:grid-cols-12 gap-8'>
            ...
            <div className='relative lg:col-span-5 rounded-lg overflow-hidden'>
              <div className='bg-white rounded-lg shadow-sm border border-gray-200 p-6 pt-1'>
                ...
                <button onClick={()=>{toast.promise(saveResume,{loading:"Saving Resume..."})}} className='bg-gradient-to-br from-green-100 to-green-200 ring-green-300 text-green-600 ring hover:ring-green-400 transition-all rounded-md px-6 py-2 mt-6 text-sm'>
                  Save Changes
                </button>
              </div>
            </div>
            ...
            <div className='lg:col-span-7 max-lg:mt-6'>
              <div className='relative w-full'>
                <div className='absolute bottom-3 left-0 right-0 flex items-center justify-center gap-2'>
                  ...
                  <button onClick={changeResumeVisibility} className='flex items-center p-2 px-4 gap-2 text-xs bg-gradient-to-br from-purple-100 to-purple-200 ring-purple-300 text-purple-600 hover:ring transition-colors rounded-lg'>
                    {resumeData.public ? <EyeIcon className='size-4'/> : <EyeOffIcon className='size-4'/>}
                    {resumeData.public ? 'Public' : 'Private'}
                  </button>
                  <button onClick={downloadResume} className='flex items-center px-4 py-2 gap-2 text-xs bg-gradient-to-br from-green-100 to-green-200 ring-green-300 text-green-600 hover:ring transition-colors rounded-lg'>
